...
def generate_slr_table(lr0_output):
    # Implementación de la generación de la tabla SLR
    # lr0 is a tuple (estados, transiciones) generated by the function lr0(grammar)
    states, transitions = lr0_output
    slr_table = defaultdict(dict)

    # Transiciones de desplazamiento
    for state, symbol, next_state in transitions:
        if symbol in grammar.terminals:
            slr_table[states.index(state)][symbol] = ("s", states.index(next_state))

    # Transiciones de reducción
    for i, state in enumerate(states):
        for item in state:
            if item.lookahead == len(item.right):  # Si es un item de reducción
                for symbol in follow_sets[
                    item.left
                ]:  # follow_sets calculado previamente
                    try:
                        rule_index = grammar.rules.index((item.left, item.right))
                    except ValueError:
                        print(f"Could not find rule: {item.left} -> {item.right}")
                        print("Current grammar rules:")
                        for rule in grammar.rules:
                            print(rule)
                        continue

                    if symbol in slr_table[i] and slr_table[i][symbol][0] == "s":
                        slr_table[i][symbol] += (
                            "r",
                            rule_index,
                        )
...
def parse_slr(slr_table, tokens):
    # Implementación del algoritmo de parseo SLR
    # tokens es una lista de strings, cada uno representando un token de entrada
    stack = [0]  # Pila de estados, el estado inicial es 0
    ...
    while True:
        top = stack[-1]
        token = tokens[i]
        action = slr_table[top].get(token)

        if action is None:
            return False, f"Error: acción indefinida (estado: {top}, token: {token})"

        if action[0] == "s":
            stack.append(action[1])
            i += 1
        elif action[0] == "r":
            _, left, right = grammar.rules[action[1]]
            stack = stack[: -len(right)]
            stack.append(slr_table[stack[-1]][left][1])
        elif action[0] == "acc":
            return True, "Entrada aceptada"
        else:
            return False, f"Error: acción desconocida {action[0]}"


def report_errors(errors):
    # Implementación del reporte de errores
    # errors es una lista de strings, cada uno representando un mensaje de error
    for error in errors:
        print(f"Error: {error}")


def handle_slr_conflicts(slr_table):
    # Implementación del manejo de conflictos en la tabla SLR
    conflicts = []
    for state, transitions in slr_table.items():
        for symbol, actions in transitions.items():
            if len(actions) > 1:
                conflicts.append((state, symbol, actions))
    return conflicts
...
if __name__ == "__main__":
    ...
    # Generar tabla SLR
    slr_table = generate_slr_table(lr0(grammar))
    if slr_table:
        print("\nTabla SLR generada correctamente.")
    else:
        print("\nError al generar la tabla SLR.")

    # Parseo SLR
    tokens = []  # Ingresar los tokens a parsear
    parse_result = parse_slr(slr_table, tokens)
    if parse_result:
        print("\nParseo SLR exitoso.")
    else:
        print("\nError en el parseo SLR.")

    # Reporte de errores
    errors = []  # Lista de errores a reportar
    if errors:
        print("\nReporte de errores:")
    ...
    # Manejo de conflictos en la tabla SLR
    handle_result = handle_slr_conflicts(slr_table)
    if handle_result:
        print("\nManejo de conflictos en la tabla SLR exitoso.")
    else:
        print("\nError en el manejo de conflictos en la tabla SLR.")
